...
let tools = document.getElementsByClassName("img-tools");
for (var i = 0; i < tools.length; i++) {  
  tools[i].addEventListener('click',ChangeTool,false);
}

// Change Color and strok
colors = document.getElementsByClassName('color');
for (var i = 0; i < colors.length; i++) {
  //colors[i].style.background =colors[i].target.myColor;
  
 colors[i].addEventListener('click',renderColors,false);
 colors[i].style.background = colors[i].getAttribute("myColor");
}
function renderColors(ev){
  //myCanvas.style.backgroundImage = "url('"+ev.target.src+"')";
  fillColor = ev.target.getAttribute("myColor");
  strokeColor =ev.target.getAttribute("myColor");
  setupCanvas();
  console.log(fillColor);
  console.log(ev.target.getAttribute("myColor"));
}
//change tool
function ChangeTool(ev){
  console.log("tool Clicked");
    ...
    document.getElementById("brush").classList.remove("selected") ;
    document.getElementById("line").classList.remove("selected");
    document.getElementById("rectangle").classList.remove("selected");
    document.getElementById("circle").classList.remove("selected");
    document.getElementById("ellipse").classList.remove("selected");
    document.getElementById("polygon").classList.remove("selected")
    // Highlight the last selected tool on toolbar
    document.getElementById(ev.target.getAttribute("id")).classList.add( "selected");
    // Change current tool used for drawing
    currentTool = ev.target.getAttribute("id");
}
...
// add listener to Select Line withe
var lineWith = document.getElementById("line-Width");
lineWith.addEventListener("change", function() {
    line_Width =lineWith.value;
    setupCanvas();
});

let startLeaveMinute ;
let startEnterMinute = new Date();
var Now ;
var doc  = document;
doc.addEventListener("mouseleave",function(){
     startLeaveMinute = new Date();
     var timer = setInterval(() => {
        startEnterMinute = new Date();
        let timeDiff = Math.abs(startEnterMinute.getTime() - startLeaveMinute.getTime());
       if( (Math.ceil(timeDiff /(1000*60)))==5){
            console.log("تجاوزت 2 دقائق !!!");
            let myNotification = new Notification('تذكير', {
                      body: "تجاوزت 5 دقائق !!!"
                     });
        clearInterval(timer);
        }
